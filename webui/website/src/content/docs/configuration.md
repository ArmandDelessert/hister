...
| `max_embedding_concurrency` | int               | `10`                                   | Maximum number of embedding requests sent to the endpoint simultaneously. Increase for fast remote endpoints, decrease (e.g. `1`–`3`) for resource-constrained local models. Set to `0` for no limit (all requests fire in parallel).                          |
